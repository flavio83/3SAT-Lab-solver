package com.threesat.fourteenth;
...
//static String filename = "C:\\DEV\\3sat\\UUF250.1065.100\\uuf250-043.cnf";
		...
		timer.schedule(new TimerTask() { 
		   ...
		   public void run() {
			  // System.out.println("a------------> " + entriesSortedByValues(addedClause));
			   ...
			   System.out.println("newClausesSize: " + newClauses.size());
		   }
		},  5000, 5000);
	}
	...
	public static List<Clause> expression = new ArrayList<Clause>();
	...
	public static ExecutorService pool = Executors.newFixedThreadPool(24,threadFactory);
	
	static long cccc = 0;
	static AtomicLong counter = new AtomicLong();
	static AtomicLong submitted = new AtomicLong();
	
	public static class ExecMethodCallable implements Callable {
		
		private Map<Clause,List<Clause>> map = null;
		
		public ExecMethodCallable(Clause c, List<Clause> expression) {
		  this.map = new HashMap<>();
		  map.put(c, expression);
		}
		
		public ExecMethodCallable(Map<Clause,List<Clause>> map) {
			  this.map = map;
			}
		...
		public Integer call() {
			...
			/*
			List<ExecMethodCallable> execlist = new ArrayList<>();
			for(Clause right : expression) {
				Clause junction = clause.simplify(right);
				if(junction!=null) {
					List<Clause> expcopy = getExpressionCopy(this.expression);
					expcopy.remove(right);
					//System.out.println(newClauses.size());
					execlist.add(new ExecMethodCallable(junction,expcopy));
					submitted.incrementAndGet();
				}
			}*/
			
			List<Map<Clause,List<Clause>>> list = new ArrayList<>();

			Iterator<Clause> it = map.keySet().iterator();
			while(it.hasNext()) {
				Clause key = it.next();
				Map<Clause,List<Clause>> aux = doit(key,map.get(key));
				list.add(aux);
				//map.remove(key);
			}
			
			for(int i=0;i<100;i++) {
				Map<Clause,List<Clause>> map = list.get(i);
				list.remove(i);
			...
			for(Map<Clause,List<Clause>> m : list) {
				pool.submit(new ExecMethodCallable(m));
			}
		...
		private Map<Clause,List<Clause>> doit(Clause clause, List<Clause> expression) {
			Map<Clause,List<Clause>> aux = new HashMap<>();
			for(Clause right : expression) {	
				Clause junction = clause.simplify(right);
				if(junction!=null) {
					System.out.println("jun-> " + junction);
					List<Clause> expcopy = getExpressionCopy(expression);
					expcopy.remove(right);
					aux.put(junction, expcopy);
				}
			}
			return aux;
		}
			...
			if(cc!=null && !used.contains(cc) && c!=null) {
				//System.out.println(c + " " + cc);
					...
					if(aux!=null) {
						...
						//auxxx.addAll(used);
						List<Clause> auxxx = getExpressionCopy(used);
						auxxx.add(cc);
						simplifier(aux,used);
	...
	public static List<Clause> getExpressionCopy(List<Clause> listToCopy) {
		return Lists.newArrayList(listToCopy);
	}
	
	public static List<Clause> newClauses = new CopyOnWriteArrayList<Clause>();
	...
	public Main3SATSolver(Clause... expression) {
		...
		do {
			...
			this.expression.addAll(newClauses);
			newClauses.clear();
			
			for(Clause clause : this.expression) {	
				List<Clause> expcopy = getExpressionCopy(this.expression);
				expcopy.remove(clause);
				pool.submit(new ExecMethodCallable(clause,expcopy));
			}
			...
			*/
			...
			System.out.println("round completed; newClauses size: " + newClauses.size());
