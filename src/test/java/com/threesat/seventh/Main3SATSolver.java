...
public class Main3SATSolver {
	...
	public List<Clause> solve(List<Clause> expression) {
		System.out.println("new round");
		//System.out.println(expression.length);
		//expression = Arrays.copyOfRange(expression, 0, 6);
		Generator<Clause> clauseGen = Factory.createSimpleCombinationGenerator(Factory.createVector(expression),3);
		//System.out.println("->"+binomial(expression.length,3));
		long counter = 0;
		...
		for (ICombinatoricsVector<Clause> vcombination : clauseGen) {
			...
			Clause c = mergeClauses(vcombination.getVector());
			if(c!=null)
			newc.add(c);
			counter++;
		}
		...
		System.out.println("3length list release: " + lexpression.size() + " vs: " + expression.size());
		return lexpression;
	}
	
	
	//refactor this method <---
	public Clause mergeClauses(List<Clause> clause) {
		List<String> literal = new ArrayList<>();
		if((clause.get(0).clauseMatchLength(clause.get(1))>0 && clause.get(0).clauseMatchLength(clause.get(2))>0) ||
				(clause.get(0).clauseMatchLength(clause.get(1))>0 && clause.get(1).clauseMatchLength(clause.get(2))>0) ||
					(clause.get(0).clauseMatchLength(clause.get(2))>0 && clause.get(2).clauseMatchLength(clause.get(1))>0)) {
				...
				for(String l : literal) {
					if(l.contains("'") && literal.contains(l.replace("'", ""))) {
						toRemove.add(l);
						toRemove.add(l.replace("'", ""));
					} 
				}
				literal.removeAll(toRemove);	
				//System.out.println(clause + " - " + literal + " - " + toRemove);
				if(literal.size()<4) {
					//System.out.println("MATCH: " + clause + " ---> " + literal);
	...
	public List<Clause> solvePerTwo(List<Clause> expression) {
		System.out.println("new round");
		//System.out.println(expression.length);
		//expression = Arrays.copyOfRange(expression, 0, 6);
		...
		for (ICombinatoricsVector<Clause> vcombination : clauseGen) {
			//System.out.println(vcombination);
			Clause c = mergeClausesPerTwo(vcombination.getVector());
			if(c!=null)
			newc.add(c);
			counter++;
		}
		...
		System.out.println("2length list release: " + lexpression.size() + " vs: " + expression.size());
		return lexpression;
	}
	
	
	public Clause mergeClausesPerTwo(List<Clause> clause) {
		List<String> literal = new ArrayList<>();
		if(clause.get(0).clauseMatchLength(clause.get(1))>0) {
	...
	private BigDecimal calculatePermutationWithNoRepetiotion(int n, int r) {
		return calculatePermutationWithNoRepetiotion(new BigInteger(String.valueOf(n)),new BigInteger(String.valueOf(r)));
	}
	
	private BigDecimal calculatePermutationWithNoRepetiotion(BigInteger n, BigInteger r) {
		BigInteger divisor = fac(n);
		BigInteger dividend = fac(n.subtract(r));
		return new BigDecimal(divisor).divide(new BigDecimal(dividend));
	}
	
	 /* BigInteger factorial code */
    public static BigInteger fac(BigInteger n) {
        BigInteger f = BigInteger.ONE;              // long f = 1;
        BigInteger g = n;					        // int n
	...
	//old constructor
	public void Main3SATSolver2(Clause... expression) {
