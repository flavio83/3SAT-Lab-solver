...
public class Main3SATSolver {
	...
	List<List<Clause>> auxx = new ArrayList<>();
	
	public List<Clause> getNextClause(Clause c, List<Clause> aux) {
		aux.add(c);
				...
				for(Clause lcl : cl) {
					if(!aux.contains(lcl)) {
						if(cl.size()>1) {
							...
							//aux.addAll(getNextClause(lcl,newlist));
							aux.addAll(getNextClause(lcl,aux));
						} else {
							aux.addAll(getNextClause(lcl,aux));
						}
					} else {
	...
	public Main3SATSolver(Clause... expression) {
		...
		for(Clause clause : PAIRS.keySet()) {
			List<Clause> auxlist = new ArrayList<Clause>();
			auxlist.addAll(Arrays.asList(expression));
			auxlist.remove(clause);
			String table = printTable(clause,auxlist);
			if(t) {
				System.out.println(table);
			}
			//if(table.split("\n").length>1) {
				//System.out.print(clause + " " + PAIRS.get(clause)[1].size());
				//System.out.print(" " + PAIRS.get(clause)[2].size());
				//System.out.println(" " + PAIRS.get(clause)[3].size());
				//System.out.println(table);
			//}
		}
		...
		for(Clause clause : PAIRS.keySet()) {	
			...
			List<Clause> lclc = getNextClause(clause,new ArrayList<Clause>());
			System.out.println(lclc);
			System.out.println("-");
			
			Clause c = lclc.get(0);
			for(int i=1;i<lclc.size();i++) {
				Clause aux = c.simplify(lclc.get(i));
				if(aux!=null) 
					c = aux;
				else 
					break;
		...
		List<Clause> ll = new ArrayList<Clause>();
		Clause cf = ccc.get(0);
		for(int i=1;i<ccc.size();i++) {
			Clause aux = cf.simplify(ccc.get(i));
			if(aux!=null) 
				cf = aux;
			else 
				break;
